 ...
 *
 * Usage:
 * 1. Alice and Bob perform initial key exchange (X3DH or similar)
 * 2. Alice initializes as initiator, Bob as responder
...
public class DoubleRatchet {
  ...
  /**
   * Initialize as the conversation initiator (Alice).
   *
   * @param sharedSecret initial shared secret from key exchange
   * @param remotePublicKey Bob's initial DH public key (prekey)
   */
  public void initializeAsAlice(byte[] sharedSecret, byte[] remotePublicKey) {
   ...
   *
   * @param sharedSecret initial shared secret from key exchange
   * @param ourPrekey Bob's prekey pair that Alice used
   */
  public void initializeAsBob(byte[] sharedSecret, DHKeyPair ourPrekey) {
